...
def scan_drive_with_server(drive_path, server_addr="localhost:50051", copy_to_temp=False):
    ...
    # Step: Copy to temp if requested (prevents drive locking issues during gRPC stream)
    if copy_to_temp:
        temp_dir_created = tempfile.mkdtemp(prefix="usb_scan_")
        try:
            for root, dirs, files in os.walk(drive_path):
                rel_root = os.path.relpath(root, drive_path)
                target_root = os.path.join(temp_dir_created, rel_root) if rel_root != "." else temp_dir_created
                os.makedirs(target_root, exist_ok=True)
                for f in files:
                    shutil.copy2(os.path.join(root, f), os.path.join(target_root, f))
            target_path = temp_dir_created
        except Exception as e:
            log_info(f"Error creating temp copy: {e}")
            if temp_dir_created: shutil.rmtree(temp_dir_created)
            return []

    try:
        # 1. Execute the scan using the logic in scanner_client.py
        # This returns a list of strings representing INFECTED files
        infected_list = client.scan_directory(target_path)
        
        # 2. Re-map the results so the main script can report on CLEAN files too
        final_results = []
        for root, _, files in os.walk(target_path):
            for f in files:
                rel_file_path = os.path.relpath(os.path.join(root, f), target_path)
                
                # If the path exists in the list returned by ScannerClient, it's infected
                if rel_file_path in infected_list:
                    final_results.append(ScanResponse(rel_file_path, "INFECTED"))
                else:
                    final_results.append(ScanResponse(rel_file_path, "CLEAN"))
        
        return final_results

    except Exception as e:
        log_info(f"Scan failed: {e}")
        return []
    finally:
